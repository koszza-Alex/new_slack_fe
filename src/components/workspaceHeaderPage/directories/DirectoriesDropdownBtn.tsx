...
DirectoriesDropdownBtn.Trigger = function Trigger({
  placeholder = "Select",
}: {
  placeholder?: string;
}) {
  ...
  return (
    ...
    >
      ...
      <svg
        className={`w-[12px] h-[12px] transition-transform duration-[200ms] ${
          open ? "rotate-180" : ""
        }`}
        viewBox="0 0 20 20"
        fill="currentColor"
...
DirectoriesDropdownBtn.Content = function Content({ children }: { children: ReactNode }) {
  ...
  return (
    <div
      className={`absolute left-0 top-[40px] z-[9999] w-[260px] bg-[#ffffff] border border-[#1f2937] rounded-[10px] p-[12px] transition-all duration-[200ms] origin-top ${
        open
          ? "opacity-100 scale-100 translate-y-0"
          : "opacity-0 scale-95 -translate-y-[6px] pointer-events-none"
      }`}
    >
      {children}
    </div>
...
DirectoriesDropdownBtn.Search = function Search({ placeholder }: { placeholder?: string }) {
  return (
    <input
      ...
      className="w-full h-[36px] px-[10px] rounded-[8px] bg-[#ffffff] border border-[#1f2937] text-[#313131] text-[13px] outline-none focus:border-[#38bdf8] focus:ring-[1px] focus:ring-[#38bdf8]"
    />
  );
};

DirectoriesDropdownBtn.List = function List({
  label,
  icon,
}: {
  label: string;
  ...
  return (
    <div className="flex items-center gap-[8px] px-[8px] py-[6px] rounded-[6px] text-[14px] text-[#313131] hover:bg-[#e1e1e1] cursor-pointer">
      {icon && <FontAwesomeIcon icon={icon} className="w-[12px] text-[#313131] h-[12px]" />}
      <span>{label}</span>
    </div>
  );
};
...
DirectoriesDropdownBtn.Radio = function Radio({
  ...
  return (
    ...
    >
      <div
        className={`w-[16px] h-[16px] flex items-center justify-center rounded-[4px] border ${
          selected
            ? "bg-[#e1e1e1] border-[#38bdf8]"
            : "border-[#6b7280]"
        }`}
      >
        {selected && (
          <svg
            className="w-[10px] h-[10px] text-[#313131]"
            viewBox="0 0 20 20"
            fill="currentColor"
          >
      ...
      <span
        className={`text-[14px] ${
          selected ? "text-[#313131]" : "text-[#313131]"
        }`}
      >
        {icon?<FontAwesomeIcon icon={icon}/>:""}
      </span><span className="text-[#313131]">{label}</span>
    </div>
  );
};